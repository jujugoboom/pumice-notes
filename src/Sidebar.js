...
const Sidebar = ({openFile, currDir, setCurrDir, currFiles, setCurrFiles}) => {
    ...
    const mapFiles = (files) => {
        return files.length > 0 && files.map(file => file.directory && file.expanded ? (
        <>
        ...
        (<>
            <div className={file.file ? "nav-file" : 'nav-directory nav-collapsed'} onClick={file.file ? () => openFile(file.path) : () => expandDir(file)} key={file.name}>{file.name}</div>
        </>)
        );
    }
    ...
    const loadFolder = async () => {
        ...
        const currentDir = (await window.electronAPI.listDirectory(selectedDir)).map(f => {
            if (f.directory) {
                f.expanded = false;
            }
            return f;
    ...
    const expandDir = async (dir) => {
        ...
        newFiles[newFiles.findIndex((f) => f.name === dir.name && f.directory)] = dir;
    ...
    return (
        <div className="sidebar">
            <button onClick={loadFolder} className='sidebar-folder-select'>Open Folder...</button>
